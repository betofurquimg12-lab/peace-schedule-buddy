...
const AppointmentConfirmationEmail = ({ subject, body, meetLink }: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>{subject ?? 'Sessão confirmada'}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>{subject ?? 'Sessão confirmada'}</Heading>
        {(body ?? '').split('\n').map((line, i) => (
...
export const template = {
  ...
  subject: (d: Record<string, any>) => d.subject ?? 'Sessão confirmada',
  displayName: 'Confirmação de agendamento',
  previewData: {
    subject: 'Sessão confirmada — 15/01 às 14:00',
    body: 'Oi, Maria!\n\nSua sessão está confirmada para 15/01 às 14:00.\n\nAté lá!',
